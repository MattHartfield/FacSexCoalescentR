...
pHL <- as.double(args[5])	# Prob of high-sex to low-sex transition
mig <- as.double(args[6])	# Migration rate (2 NT m)
d <- as.integer(args[7])	# Number of demes in island model
if(d == 1){
...
## THEN sexL[i], sexH[i] = low-sex, high-sex rate in each deme (2*N*sigma_{H,L} in this case)
sps <- args[8:(7+4*d)]
...
# More error checking
if(any(sexL < 0) || any(sexH < 0)){
	stop("Rate of sexual reproduction has to lie above 0.")
}
...
probset2 <- function(g,sexin,mig,Nwith,Nbet){
	...
	pr[5,] <- mapply(P8,Nwith,Nbet)
	pr[6,] <- mapply(P9,Nwith,g)
	pr[7,] <- mapply(P10,Nwith,Nbet,mig)
...
coalesce <- function(itab,Tt,Nwith,Nbet,din,exin,dein,e2in,dt){
	...
	{	# Event 5: Two pre-existing unique samples re-create paired sample.
		rands <- ssamp(WH,BH,0,deme,3)	# Two combining samples
	...
	{	# Event 6: Two pre-existing unique samples coalesce
		rands <- ssamp(WH,BH,0,deme,3)	# Two coalesced samples
	...
	{	# Event 7: Two remaining paired samples doubly coalesce asexually.
		rands <- ssamp(WH,BH,2,deme,1)			# 2 other within-host samples that coalesce
	...
	{	# Event 8: One of the x - k remaining paired samples coalesces with a unique sample
		rands <- ssamp(WH,BH,1,deme,1)	# A paired sample involved in coalescence
	...
	{	# Event 10: Migration of a sample	
		if(e2in == 1){
			bhc <- ssamp(WH,BH,1,deme,1)	# Choosing the WH sample that migrates
...
rate_change <- function(pST,pLH,pHL,sexH,sexL,switch1){
...
for(i in 1:Nreps){
	...
	rcout <- rate_change(pST,pLH,pHL,sexH,sexL,0)
...
#		}else 
		...
		if(NextT > (tls + tts)){ 	# If next event happens after a switch, change rates of sex
			...
			Ttot <- tls
			rcout <- rate_change(pST,pLH,pHL,sexH,sexL,1)
			sexC <- as.double(rcout$SCNew)
			tts <- as.double(rcout$tswitch)
			pST <- as.integer(rcout$newpST)
			...
			draw <- rmultinom(1,1,rowSums(probs))[,1]
			event <- match(1,draw)
